...
INPUT_FILE = RAW_DATA_DIR / "user_job.csv"
...
def build_user_key_mapping_from_user_data(user_data_path: Path) -> pd.DataFrame:
    ...
    - For each original user_id group:
        ...
        assign seq = 1..N
        user_key = <digits>-<seq>
          e.g., USER40678 group of 2 -> 40678-1, 40678-2
          non-dupe USER61846         -> 61846-1
    """
    ...
    mapped = mapped.drop_duplicates(subset=["user_id_orig", "name"])

    return mapped


def main():
    print(f"Reading user_job: {INPUT_FILE}")
    ...
    if not mapping.empty:
        ...
        print(f"Assigned user_key for {num_with_key} user_job rows.")
        ...
        # If you really want to enforce having user_key, you could assert here.
    # --------------------------------------------------------- #
